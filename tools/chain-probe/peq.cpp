// Vendored PressureEqualizer.cpp body under the peq.hpp shim.
#include "peq.hpp"
...
// fast_float::from_chars replacement: strtof over a bounded copy.
namespace fast_float {
struct from_chars_result_t { const char *ptr; };
inline from_chars_result_t from_chars(const char *first, const char *last, float &out) {
    char buf[64];
    ...
    buf[n] = 0;
    char *end2 = nullptr;
    out = std::strtof(buf, &end2);
    return from_chars_result_t{first + (end2 - buf)};
}
} // namespace fast_float
...
bool PressureEqualizer::process_line(const char *line, const char *line_end, GCodeLine &buf)
{
    ...
    const bool found_extrude_set_speed_tag = str_line.find(EXTRUDE_SET_SPEED_TAG) != std::string::npos;
    const bool found_extrude_end_tag = str_line.find(EXTRUDE_END_TAG) != std::string::npos;
...
void PressureEqualizer::push_line_to_output(const size_t line_idx, float new_feedrate, const char *comment)
{
    ...
    for (size_t axis_idx = 0; axis_idx < 3; ++axis_idx)
        if (line.pos_provided[axis_idx])
            extrusion_formatter.emit_axis(char('X' + axis_idx), line.pos_end[axis_idx], 3);
    extrusion_formatter.emit_axis('E', m_use_relative_e_distances ? (line.pos_end[3] - line.pos_start[3]) : line.pos_end[3], 5);
