...
#include <cassert>
#include <cmath>
template <typename T> static inline T next_highest_power_of_2(T v) { if (v != 0) --v; v |= v >> 1; v |= v >> 2; v |= v >> 4; v |= v >> 8; v |= v >> 16; v |= v >> 32; return ++v; }
...
public:
    ...
    void build(std::vector<size_t> &indices)
    {
        if (indices.empty())
            ...
            m_nodes.assign(next_highest_power_of_2(indices.size() + 1) * 4, npos); // probe: pad for heap-indexed build overflow;
