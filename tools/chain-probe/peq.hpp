// Vendored PressureEqualizer.hpp class body with the libslic3r.h /
// PrintConfig.hpp dependencies replaced by the minimal standalone shim.
#ifndef slic3r_GCode_PressureEqualizer_shim_hpp_
#define slic3r_GCode_PressureEqualizer_shim_hpp_
...
struct LayerResult {
    std::string gcode;
    bool nop_layer_result = true;
    static LayerResult make_nop_layer_result() { return LayerResult{}; }
};

struct MiniConfig {
    struct FloatOpt { double value; };
    struct BoolOpt { bool value; };
    struct FloatsOpt { std::vector<double> values; };
    FloatOpt max_volumetric_extrusion_rate_slope{0.};
    FloatOpt max_volumetric_extrusion_rate_slope_segment_length{2.};
    BoolOpt extrusion_rate_smoothing_external_perimeter_only{false};
    BoolOpt use_relative_e_distances{false};
    FloatsOpt filament_diameter{};
};
using GCodeConfig = MiniConfig;

enum class ExtrusionRole : int {
    erNone, erPerimeter, erExternalPerimeter, erOverhangPerimeter, erInternalInfill, erSolidInfill,
    erTopSolidInfill, erBottomSurface, erIroning, erBridgeInfill, erInternalBridgeInfill, erGapFill,
    erSkirt, erBrim, erSupportMaterial, erSupportMaterialInterface, erSupportTransition,
    erWipeTower, erCustom, erMixed, erCount
};

// GCodeG1Formatter with GCodeWriter.hpp's quantize + trailing-zero trim.
class GCodeG1Formatter {
public:
    GCodeG1Formatter() { m_buf[0] = 'G'; m_buf[1] = '1'; m_len = 2; }
    void emit_axis(char axis, double v, size_t digits) {
        double factor = 1.;
        for (size_t i = 0; i < digits; ++i) factor *= 10.;
        double q = std::round(v * factor) / factor;
        char tmp[64];
        std::snprintf(tmp, sizeof tmp, "%.*f", int(digits), q);
        std::string s = tmp;
        while (!s.empty() && s.back() == '0') s.pop_back();
        if (!s.empty() && s.back() == '.') s.pop_back();
        m_buf[m_len++] = ' '; m_buf[m_len++] = axis;
        for (char c : s) m_buf[m_len++] = c;
    }
    void emit_e(double v) { emit_axis('E', v, 5); }
    void emit_f(double v) { emit_axis('F', v, 3); }
    void emit_string(const std::string &s) { for (char c : s) m_buf[m_len++] = c; }
    std::string string() { m_buf[m_len] = 0; return std::string(m_buf) + "\n"; }
private:
    char m_buf[1024]{};
    size_t m_len = 0;
};
