// Vendored CoolingBuffer.cpp body under cbshim.hpp (probe7).
#include "cbshim.hpp"
#include <cassert>
#include <cstring>
...
void CoolingBuffer::reset(const Vec3d &position)
{
    ...
    m_current_pos[4] = float(m_config.travel_speed.get_at(0));
    m_fan_speed = -1;
...
std::string CoolingBuffer::process_layer(std::string &&gcode, size_t layer_id, bool flush)
{
    ...
    std::string out;
    std::fprintf(stderr, "PL enter len=%zu\n", m_gcode.size());
    if (flush) {
        std::fprintf(stderr, "PL parse...\n");
        ...
        std::fprintf(stderr, "PL parsed, slowdown...\n");
        for (const auto &a : per_extruder_adjustments) {
            std::fprintf(stderr, "ADJ extr=%u cool=%d lines=%zu\n", a.extruder_id, (int)a.cooling_slow_down_enabled, a.lines.size());
            for (const auto &l : a.lines)
                std::fprintf(stderr, "P7L type=%x len=%.6f f=%.4f t=%.9f tmax=%.9f\n", l.type, l.length, l.feedrate, l.time, l.time_max);
        }
        float layer_time_stretched = this->calculate_layer_slowdown(per_extruder_adjustments);
        std::fprintf(stderr, "PL slowed %f, cooldown...\n", layer_time_stretched);
        out = this->apply_layer_cooldown(m_gcode, layer_id, layer_time_stretched, per_extruder_adjustments);
        m_gcode.clear();
    }
    std::fprintf(stderr, "PL done\n");
    return out;
}
...
std::vector<PerExtruderAdjustments> CoolingBuffer::parse_layer_gcode(const std::string &gcode, std::vector<float> &current_pos) const
{
    ...
    for (; *line_start != 0; line_start = line_end)
    {
        ...
        if (shim_starts_with(sline, "G0 "))
            line.type = CoolingLine::TYPE_G0;
        else if (shim_starts_with(sline, "G1 "))
            line.type = CoolingLine::TYPE_G1;
        else if (shim_starts_with(sline, "G92 "))
            line.type = CoolingLine::TYPE_G92;
        else if (shim_starts_with(sline, "G2 "))
            line.type = CoolingLine::TYPE_G2;
        else if (shim_starts_with(sline, "G3 "))
        ...
        if (line.type) {
            ...
            bool external_perimeter = shim_contains(sline, ";_EXTERNAL_PERIMETER");
            bool wipe               = shim_contains(sline, ";_WIPE");
            ...
            if (shim_contains(sline, ";_EXTRUDE_SET_SPEED"))
            ...
            if (shim_contains(sline, ";_EXTRUDE_SET_SPEED") && ! wipe && adjust_external) {
            ...
            if ((line.type & CoolingLine::TYPE_G92) == 0) {
                ...
                if (line.type & CoolingLine::TYPE_G2 || line.type & CoolingLine::TYPE_G3) {
                    ...
                    float dxy = shim_arc_length(start, end, center, is_ccw);
                    dxy2 = dxy * dxy;
                } else {
        ...
        } else if (shim_starts_with(sline, ";_EXTRUDE_END")) {
            line.type = CoolingLine::TYPE_EXTRUDE_END;
            active_speed_modifier = size_t(-1);
        } else if (shim_starts_with(sline, m_toolchange_prefix.c_str())) {
            unsigned int new_extruder = 0;
            try {
                new_extruder = (unsigned)std::stoul(sline.substr(m_toolchange_prefix.size()));
        ...
        } else if (shim_starts_with(sline, ";_OVERHANG_FAN_START")) {
            line.type = CoolingLine::TYPE_OVERHANG_FAN_START;
        } else if (shim_starts_with(sline, ";_OVERHANG_FAN_END")) {
            line.type = CoolingLine::TYPE_OVERHANG_FAN_END;
        } else if (shim_starts_with(sline, ";_INTERNAL_BRIDGE_FAN_START")) { // ORCA: Add support for separate internal bridge fan speed control
            line.type = CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_START;
        } else if (shim_starts_with(sline, ";_INTERNAL_BRIDGE_FAN_END")) { // ORCA: Add support for separate internal bridge fan speed control
            line.type = CoolingLine::TYPE_INTERNAL_BRIDGE_FAN_END;
        } else if (shim_starts_with(sline, ";_SUPP_INTERFACE_FAN_START")) {
            line.type = CoolingLine::TYPE_SUPPORT_INTERFACE_FAN_START;
        } else if (shim_starts_with(sline, ";_SUPP_INTERFACE_FAN_END")) {
            line.type = CoolingLine::TYPE_SUPPORT_INTERFACE_FAN_END;
        } else if (shim_starts_with(sline, ";_IRONING_FAN_START")) { // ORCA: Add support for ironing fan speed control
            line.type = CoolingLine::TYPE_IRONING_FAN_START;
        } else if (shim_starts_with(sline, ";_IRONING_FAN_END")) { // ORCA: Add support for ironing fan speed control
            line.type = CoolingLine::TYPE_IRONING_FAN_END;
        } else if (shim_starts_with(sline, "G4 ")) {
            // Parse the wait time.
        ...
        } else if (shim_starts_with(sline, ";_FORCE_RESUME_FAN_SPEED")) {
    ...
    std::vector<PerExtruderAdjustments>    &per_extruder_adjustments)
{
    ...
    const unsigned int part_cooling_fan_min_pwm = static_cast<unsigned int>(std::max(0.f, float(m_config.part_cooling_fan_min_pwm.get_at(0))));
...
#undef EXTRUDER_CONFIG
        ...
        if (fan_speed_new != m_fan_speed) {
            ...
            if (immediately_apply)
                new_gcode  += shim_set_fan(m_fan_speed, part_cooling_fan_min_pwm);
        }
        //BBS
        if (additional_fan_speed_new != m_additional_fan_speed) {
            m_additional_fan_speed = additional_fan_speed_new;
            if (immediately_apply && m_config.auxiliary_fan.get_at(0))
                new_gcode += shim_set_additional_fan(m_additional_fan_speed);
        }
    };
    ...
    for (const CoolingLine *line : lines) {
        ...
        if (line->type & CoolingLine::TYPE_SET_TOOL) {
            unsigned int new_extruder = 0;
            try { std::string t(line_start + m_toolchange_prefix.size(), line_end); new_extruder = (unsigned)std::stoul(t);
            ...
            if (m_additional_fan_speed != -1 && m_config.auxiliary_fan.get_at(0))
                new_gcode += shim_set_additional_fan(m_additional_fan_speed);
        }
            ...
            if (end < line_end) {
                if (line->type & (CoolingLine::TYPE_ADJUSTABLE | CoolingLine::TYPE_EXTERNAL_PERIMETER | CoolingLine::TYPE_WIPE)) {
                    ...
                    shim_replace_all(comment, ";_EXTRUDE_SET_SPEED", "");
                    if (line->type & CoolingLine::TYPE_EXTERNAL_PERIMETER)
                        shim_replace_all(comment, ";_EXTERNAL_PERIMETER", "");
                    if (line->type & CoolingLine::TYPE_WIPE)
                        shim_replace_all(comment, ";_WIPE", "");
                    new_gcode += comment;
                } else {
        ...
        if (need_set_fan) {
            if (fan_speed_change_requests[CoolingLine::TYPE_OVERHANG_FAN_START]){
                new_gcode += shim_set_fan(overhang_fan_speed, part_cooling_fan_min_pwm);
                ...
                new_gcode += shim_set_fan(internal_bridge_fan_speed, part_cooling_fan_min_pwm);
                ...
                new_gcode += shim_set_fan(supp_interface_fan_speed, part_cooling_fan_min_pwm);
                m_current_fan_speed = supp_interface_fan_speed;
            }
            else if (fan_speed_change_requests[CoolingLine::TYPE_IRONING_FAN_START]){
                new_gcode += shim_set_fan(ironing_fan_speed, part_cooling_fan_min_pwm);
                ...
                new_gcode += shim_set_fan(m_current_fan_speed, part_cooling_fan_min_pwm);
