...
6. Port G-code processing for printing-time metadata, M73 insertion, and filament statistics. Normalize only the known-indeterminate Orca BBL-path object-comment ID field at the test seam while retaining deterministic Ares IDs; then remove the golden test ignore and require normalized byte equality. Commit and push each independently verified slice.
