...
5. Add alignment tests before writes, prove O18 `None` slots remain `None` without shifting populated indices, snapshot all O18 allocations, and prove no source/cache aliasing. Add zero-O19-invocation witnesses for global spiral, counterbore, multi-region, O17 interface-shell/active-extra-bridge, and an O17 geometry failure. Require the consuming O19 `prepare` path for top and bottom errors plus separate direct-success disposal and public-incomplete consumption on 64-KiB stacks with both traversal/hierarchy trees at depth 10,000, weak/drop-probe confirmation, exact errors, no successor, and no partial cache.
