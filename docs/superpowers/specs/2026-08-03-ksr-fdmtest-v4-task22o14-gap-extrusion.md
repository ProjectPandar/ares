...
4. Ownership tests preserve the boxed O5 address, O11 expolygon allocations, O10 collection/entity/path allocations, and all surviving O13 polyline point/width allocations. Transactional tests separately trigger invalid option, invalid derived flow, open-offset range, and difference range failures, prove no successor is observable and exact stable errors, and prove iterative cleanup with 10,000-node predecessors on the shared project-slice constrained test stack (64 KiB on Unix, 256 KiB on Windows).
