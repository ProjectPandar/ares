...
4. Ownership tests preserve the boxed O5 address, O11 expolygon allocations, O10 collection/entity/path allocations, and all surviving O13 polyline point/width allocations. Transactional tests separately trigger invalid option, invalid derived flow, open-offset range, and difference range failures, prove no successor is observable and exact stable errors, and prove iterative cleanup with 10,000-node predecessors on a 64 KiB Unix stack and a 256 KiB Windows stack. The larger Windows allowance is specific to the O14 open-offset/variable-width success-and-error execution frames, not a platform-wide Clipper baseline; unrelated constrained-stack tests retain 64 KiB unless their own target-runner evidence requires otherwise.
