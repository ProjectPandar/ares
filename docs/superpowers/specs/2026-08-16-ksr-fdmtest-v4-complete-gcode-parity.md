...
The `ares slice` project route accepts `tests/ksr_fdmtest_v4/ksr_fdmtest_v4.project.3mf` and emits the complete `ksr_fdmtest_v4.gcode` byte stream. The only permitted difference is the single generator metadata line: `OrcaSlicer` becomes `Ares`, and its date/time may differ. Printing-time metadata is output behavior, not timestamp metadata, and must match.
...
6. `src/libslic3r/GCode.cpp` export finalization — executable block termination and filament statistics.
