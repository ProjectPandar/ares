...
3. Failure hooks cover hole intersection, shell union, top/bottom anchor offset and intersection independently. Whole-project tests prove exact error text, no partial successor, stage-before-move, and iterative cleanup for both predecessor tree families at depth 10,000 on constrained test stacks (64 KiB on Unix, 256 KiB on Windows). Direct-success and public-incomplete disposal get separate witnesses. Unchanged exact errors and zero O20 invocation are required for spiral, counterbore, multi-region, interface shells, active extra bridge, an O17 geometry failure, and both O19 top/bottom offset failures.
