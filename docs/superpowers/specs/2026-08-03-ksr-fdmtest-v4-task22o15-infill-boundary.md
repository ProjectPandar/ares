...
- Success and every stable error path use iterative sinks and must fit the constrained test stack (64 KiB on Unix, 256 KiB on Windows) with both predecessor tree families deepened to 10,000 nodes.
...
5. Ownership tests preserve O14/O13/O11/O10 and boxed O5 allocations. Success plus overlap error and each independently injected geometry failure clean up iteratively on the constrained test stack (64 KiB on Unix, 256 KiB on Windows).
