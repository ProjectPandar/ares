...
fn passes_short_sets_through() {
    ...
    let out = connect_lines_using_hooks(single, &test_boundary(), 100.0, 500.0, 500.0, 40.0);
    assert_eq!(out.len(), 1);
    assert!(
        connect_lines_using_hooks(Vec::new(), &test_boundary(), 100.0, 500.0, 500.0, 40.0)
            .is_empty()
    );
}
...
fn zero_hook_length_is_open() {
    ...
    let out = connect_lines_using_hooks(lines, &test_boundary(), 100.0, 0.0, 0.0, 40.0);
    assert_eq!(out.len(), 2);
}
...
fn t_joint_gets_hook_point() {
    ...
    let out = connect_lines_using_hooks(lines, &test_boundary(), 100.0, 500.0, 500.0, 40.0);
    assert!(
