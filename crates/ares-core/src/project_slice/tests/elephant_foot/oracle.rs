...
#[test]
fn task22m_oracle_freezes_tiny_union_seam_and_two_pass_order() {
    let tiny = rectangle(0, 0, 1_000_000, 1_000_000);
    let geometry = derive_geometry(
        DEFAULT_MINIMUM_WIDTH,
        stage_compensation(CoordinateScale::Normal),
        CoordinateScale::Normal,
    )
    .unwrap();
    assert_eq!(compensate_expolygon(&tiny, geometry), Ok(tiny.clone()));
    assert_eq!(
        compensate(&[tiny], CoordinateScale::Normal),
        [expolygon(
            &[
                (1_000_000, 1_000_000),
                (0, 1_000_000),
    ...
    let expected = vec![
        expolygon(&[(60, 60), (0, 60), (0, 0), (60, 0)], &[&left_hole]),
        expolygon(&[(40, 40), (20, 40), (20, 20), (40, 20)], &[]),
        expolygon(&[(160, 60), (100, 60), (100, 0), (160, 0)], &[&right_hole]),
    ];
    assert_eq!(compensate(&input, CoordinateScale::Normal), expected);
    assert_eq!(compensate(&input, CoordinateScale::Normal), expected);
}

#[test]
fn task22m_elephant_foot_freezes_natural_result_count_fallbacks() {
    ...
    assert_eq!(
        compensate_expolygons(&[dumbbell], minimum_width, 0.15, CoordinateScale::Normal,).unwrap(),
        [expolygon(
            &[
                (10_000_000, 4_950_000),
                (20_000_000, 4_950_000),
    ...
    assert_eq!(
        compensate_expolygons(&[ring], minimum_width, 0.15, CoordinateScale::Normal).unwrap(),
        [expolygon(
            &[
                (20_000_000, 20_000_000),
                (0, 20_000_000),
