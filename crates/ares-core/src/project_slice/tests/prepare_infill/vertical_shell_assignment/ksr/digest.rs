use crate::{
    ...
    project_slice::{
        prepare_infill::{
            surface_type_detection::PreparedSurfaceTypeObject, vertical_shell_assignment,
        },
...
pub(super) fn record_digests(objects: &[PreparedSurfaceTypeObject]) -> Vec<i128> {
    objects
        ...
        .map(|record| {
            let mut digest = 0x0052_4543_4f52_445f_4449_4745_5354_i128;
...
pub(super) fn record_sequence_digest(
    ...
    assert_eq!(positions.len(), active.len());
    assert_eq!(active.len(), before.len());
    assert_eq!(active.len(), after.len());
    let mut digest = 0x0052_4543_4f52_445f_5345_5155_454e_4345_i128;
    mix(&mut digest, active.len() as i128);
    for (index, (((&(object, slot), &active), &before), &after)) in positions
        .iter()
        .zip(active)
        .zip(before)
        .zip(after)
...
pub(super) fn event_sequence_digest(events: &[vertical_shell_assignment::GeometryStep]) -> i128 {
    let mut digest = 0x4556_454e_545f_5345_5155_454e_4345_i128;
