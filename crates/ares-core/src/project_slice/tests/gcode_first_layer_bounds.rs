...
#[tokio::test]
async fn bounds_keep_distance_fallback_without_a_generated_skirt_plan() {
    let mut archive = KsrArchive::new();
    let mut settings: serde_json::Value =
        serde_json::from_str(&archive.entry_text(PROJECT_SETTINGS)).unwrap();
    let settings = settings.as_object_mut().unwrap();
    settings.insert(
        "machine_start_gcode".to_owned(),
        json!(";FALLBACK {first_layer_print_min[0]},{first_layer_print_min[1]} {first_layer_print_max[0]},{first_layer_print_max[1]}"),
    );
    settings.insert("machine_end_gcode".to_owned(), json!(";END"));
    settings.insert("skirt_loops".to_owned(), json!("0"));
    settings.insert("skirt_height".to_owned(), json!("1"));
    settings.insert("skirt_distance".to_owned(), json!("2"));
    settings.insert("min_skirt_length".to_owned(), json!("0"));
    archive.insert_text(PROJECT_SETTINGS, &serde_json::to_string(&settings).unwrap());
    ...
    assert!(
        output
            .lines()
            .any(|line| line == ";FALLBACK 93.5392,79.8921 171.639,152.992")
    );
}
