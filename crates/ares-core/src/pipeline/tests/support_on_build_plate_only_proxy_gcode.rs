...
#[test]
fn support_on_build_plate_only_removes_floating_support_gcode() {
    let off = output_for_build_plate_only(false);
    let on = output_for_build_plate_only(true);

    assert_ne!(off, on);
    assert!(off.contains(";PRINT_PATH:support_material:"));
    assert!(!on.contains(";PRINT_PATH:support_material:"));
}

fn output_for_build_plate_only(enabled: bool) -> String {
    let options = options(json!({ "support_on_build_plate_only": enabled }));
