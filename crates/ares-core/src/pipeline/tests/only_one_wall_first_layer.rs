...
#[test]
fn only_one_wall_first_layer_removes_first_layer_internal_wall_gcode() {
    ...
    assert_eq!(pipeline.layer_perimeters()[0].paths().len(), 1);
    ...
    assert!(first_layer.contains(";PERIMETER:external:0,0 -> 4,0"));
    assert!(!first_layer.contains(";PERIMETER:internal:"));
    assert!(!first_layer.contains(";PRINT_PATH:internal_perimeter:"));
