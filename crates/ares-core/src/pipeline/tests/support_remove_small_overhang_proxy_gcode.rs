...
#[test]
fn support_remove_small_overhang_removes_small_support_gcode() {
    let off = output_for_remove_small_overhang(false);
    let on = output_for_remove_small_overhang(true);
...
fn output_for_remove_small_overhang(enabled: bool) -> String {
    let options = options(json!({
        "line_width": 0.5,
        "support_remove_small_overhang": enabled
    }));
