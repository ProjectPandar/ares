...
#[test]
fn first_layer_flow_ratio_changes_first_layer_perimeter_gcode_extrusion_delta() {
    ...
    .unwrap();
    let high: SliceOptions = serde_json::from_value(json!({
        "layer_height": 0.2,
        "initial_layer_height": 0.2,
        "wall_loops": 1,
        "line_width": 0.4,
    ...
    let high_gcode =
        String::from_utf8(format_gcode(&rectangular_pipeline(&high), &high).unwrap()).unwrap();

    assert!(
        (first_extrusion_delta(&high_gcode, "external_perimeter")
            - first_extrusion_delta(&base_gcode, "external_perimeter") * 3.0)
            .abs()
            <= 0.000002
    );
}
...
#[test]
fn first_layer_flow_ratio_does_not_change_second_layer_perimeter_gcode_extrusion_delta() {
    let base: SliceOptions = serde_json::from_value(json!({
        "layer_height": 0.2,
        "initial_layer_height": 0.2,
        "wall_loops": 1,
    ...
    let base_pipeline = run_slicing_pipeline(square_pyramid_ascii_stl(), &base).unwrap();
    let high_pipeline = run_slicing_pipeline(square_pyramid_ascii_stl(), &high).unwrap();
    ...
    assert!(
        (first_layer_extrusion_delta(&high_gcode, 1, "external_perimeter")
            - first_layer_extrusion_delta(&base_gcode, 1, "external_perimeter"))
        .abs()
            <= 0.000002
    );
}
...
#[test]
fn first_layer_flow_ratio_does_not_change_first_layer_brim_gcode_extrusion_delta() {
    let base: SliceOptions = serde_json::from_value(json!({
        "layer_height": 0.2,
        "initial_layer_height": 0.2,
        "wall_loops": 0,
    ...
    assert!(
        (first_extrusion_delta(&high_gcode, "brim") - first_extrusion_delta(&base_gcode, "brim"))
            .abs()
            <= 0.000002
    );
}
