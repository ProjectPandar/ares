...
#[test]
fn tree_auto_critical_regions_only_removes_support_gcode() {
    let off = output_for_critical_regions_only(false);
    let on = output_for_critical_regions_only(true);
...
fn output_for_critical_regions_only(enabled: bool) -> String {
    let options = options(json!({
        "support_type": "tree(auto)",
        "tree_support_auto_brim": false,
        "support_critical_regions_only": enabled
    }));
