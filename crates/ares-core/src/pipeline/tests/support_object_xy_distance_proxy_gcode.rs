...
#[test]
fn support_object_xy_distance_changes_emitted_support_coordinates() {
    let zero = output_for_distance(0.0);
    let positive = output_for_distance(0.5);
...
fn output_for_distance(distance: f64) -> String {
    let options = options(json!({
        "support_object_xy_distance": distance,
        "support_remove_small_overhang": false
    }));
    let layers = vec![Layer::new(7, 0.2, 1.6)];
...
fn pipeline(input: PipelineInput) -> SlicingPipeline {
    SlicingPipeline {
        ...
        layers: input.layers,
        layer_slices: vec![LayerSlice::new(7, 1.6, Vec::new())],
        layer_contours: input.layer_contours,
        layer_perimeters: vec![LayerPerimeters::new(7, 1.6, Vec::new())],
        layer_gap_fills: vec![LayerGapFills::new(7, 1.6, Vec::new())],
        layer_infills: vec![LayerInfills::new(7, 1.6, Vec::new())],
        layer_skirts: vec![LayerSkirts::new(7, 1.6, Vec::new())],
        layer_brims: vec![LayerBrims::new(7, 1.6, Vec::new())],
