...
#[test]
fn filament_retract_before_wipe_zero_moves_speed_allowed_retraction_into_wipe() {
    let output = role_output(
        json!({
            "wipe": true,
            "wipe_distance": 0.25,
            "retract_before_wipe": 100,
            "filament_retract_before_wipe": [0, 100]
        }),
    ...
    assert!(!output.contains("G1 E-0.8 F1800 ; retract"));
    assert!(!output.contains("G1 X0.75 Y0 F3600 ; wipe and retract"));
}

#[test]
fn filament_retract_before_wipe_full_moves_all_retraction_before_wipe() {
    let output = role_output(
        json!({
            "wipe": true,
            "wipe_distance": 0.5,
    ...
    assert!(!output.contains("G1 X0.5 Y0 E-0.4 F3600 ; wipe and retract"));
}

#[test]
fn serialized_filament_retract_before_wipe_uses_first_percent() {
    let output = role_output(
        json!({
            "wipe": true,
            "wipe_distance": 0.25,
...
#[test]
fn nil_filament_retract_before_wipe_falls_back_to_unprefixed_percent() {
    for value in [json!(null), json!([null, 0]), json!("nil,0")] {
        let output = role_output(
            json!({
                "wipe": true,
                "wipe_distance": 0.5,
                "retract_before_wipe": 100,
                "filament_retract_before_wipe": value
            }),
            vec![PrintPathRole::SparseInfill, PrintPathRole::SparseInfill],
        )
        .unwrap();
        ...
        assert!(output.contains("G1 E-0.8 F1800 ; retract"));
        assert!(output.contains("G1 X0.5 Y0 F3600 ; wipe and retract"));
    }
}

#[test]
fn invalid_filament_retract_before_wipe_values_are_rejected_with_option_key() {
    for (case, value) in [
        ("empty array", json!([])),
        ("negative scalar", json!(-0.1)),
        ("above percent range", json!(100.1)),
        ("non-finite string scalar", json!("inf")),
        ("invalid string token", json!("bad")),
        ("invalid comma token", json!("50,bad")),
        ("non-number array member", json!([50, "bad"])),
        ("negative later array member", json!([50, -0.1])),
        ("above range later array member", json!([50, 100.1])),
        ...
        assert!(
            err.to_string().contains("filament_retract_before_wipe"),
            "filament_retract_before_wipe was missing from {case}: {err}"
        );
    }
}
