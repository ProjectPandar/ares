...
#[tokio::test]
async fn before_layer_change_gcode_emits_before_each_z_travel() {
    ...
    assert_line_before(&output, ";BEFORE 1 0.2", "G1 Z0.2 F7200");
    assert_line_before(&output, ";BEFORE 2 0.4", "G1 Z0.4 F7200");
}

#[tokio::test]
async fn before_layer_change_gcode_replaces_brace_layer_placeholders() {
    let output = slice_custom_gcode_output(json!({
        "before_layer_change_gcode": ";BRACE {layer_num} {layer_z} {max_layer_z}"
    }))
    .await
    .unwrap();

    assert_line_before(&output, ";BRACE 1 0.2 0.2", "G1 Z0.2 F7200");
    assert_line_before(&output, ";BRACE 2 0.4 0.4", "G1 Z0.4 F7200");
}

#[tokio::test]
async fn before_layer_change_gcode_replaces_bracket_layer_placeholders() {
    let output = slice_custom_gcode_output(json!({
        "before_layer_change_gcode": ";BRACKET [layer_num] [layer_z] [max_layer_z]"
    }))
    .await
    .unwrap();

    assert_line_before(&output, ";BRACKET 1 0.2 0.2", "G1 Z0.2 F7200");
    assert_line_before(&output, ";BRACKET 2 0.4 0.4", "G1 Z0.4 F7200");
}

#[tokio::test]
async fn before_layer_change_gcode_keeps_unknown_placeholders() {
    let output = slice_custom_gcode_output(json!({
        "before_layer_change_gcode": ";UNKNOWN {future_layer} [future_layer] [layer_num]"
    }))
    .await
    .unwrap();
    ...
    assert_line_before(
        &output,
        ";UNKNOWN {future_layer} [future_layer] 1",
        "G1 Z0.2 F7200",
    );
}

#[tokio::test]
async fn before_layer_change_gcode_rejects_invalid_values() {
...
#[tokio::test]
async fn before_layer_change_gcode_absent_or_empty_is_noop() {
