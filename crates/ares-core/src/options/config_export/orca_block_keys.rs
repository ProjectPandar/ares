//! Canonical config-block key set: the config keys OrcaSlicer 2.4.2
//! writes into the CONFIG_BLOCK for every printer (the intersection of the
//! config block over a large printer sweep). Keys outside this set only
//! appear when the loaded project defines them.

pub(crate) const ORCA_BLOCK_KEYS: [&str; 617] = [
