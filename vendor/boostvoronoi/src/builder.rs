...
use crate::beach_line::BeachlineCursor;
...
/// # Ok::<(), BvError>(())
/// ```
pub struct Builder<I: InputType> {
    pub(crate) site_events_: Vec<vse::SiteEvent<I>>,
    circle_events_: vc::CircleEventQueue,
    end_points_: BinaryHeap<vep::EndPointPair<I>>,
    pub(crate) beach_line_: vb::BeachLine<I>,
...
impl<I: InputType> Default for Builder<I> {
    fn default() -> Self {
        Self {
...
impl<I: InputType> Builder<I> {
    /// Inserts vertices.
    /// This should be done before inserting segments.
    /// This method accepts iterators of anything that implements `Into<boostvoronoi::prelude::Point>`
        ...
        IT: Copy + Into<Point<I>>,
    {
        ...
        for v in vertices.into_iter().map(|v| -> Point<I> { v.into() }) {
            let mut s = vse::SiteEvent::<I>::new(vse::Site::Point(v), self.index_);
    ...
    /// This method accepts iterators of anything that implements `Into<boostvoronoi::prelude::Line>`
        ...
        IT: Copy + Into<Line<I>>,
    {
        ...
        for line in segments.into_iter().map(|s| -> Line<I> { s.into() }) {
            ...
            let se = if line.start == line.end {
                ...
                let mut se = vse::SiteEvent::<I>::new(vse::Site::Point(line.start), self.index_);
                se.or_source_category(Cb::SINGLE_POINT__BIT);
                se
            } else {
                // the line was a line with unique endpoints
                let mut s1 = vse::SiteEvent::<I>::new(vse::Site::Point(line.start), self.index_);
                s1.or_source_category(Cb::SEGMENT_START_POINT__BIT);

                let mut s2 = vse::SiteEvent::<I>::new(vse::Site::Point(line.end), self.index_);
                ...
                if vp::point_comparison::point_comparison(line.start, line.end) {
                    let mut s3 = vse::SiteEvent::<I>::new(site, self.index_);
                    s3.or_source_category(Cb::INITIAL_SEGMENT);
                    s3
                } else {
                    let mut s3 = vse::SiteEvent::<I>::new(site.reverse(), self.index_);
    ...
    pub fn build(mut self) -> Result<vd::Diagram, BvError> {
        let mut output: vd::Diagram = vd::Diagram::new(self.site_events_.len());
        ...
        while !self.circle_events_.is_empty()
            || (site_event_iterator_ as usize != self.site_events_.len())
        {
            #[cfg(feature = "console_debug")]
            {
                tln!("################################################");
            ...
            } else if site_event_iterator_ as usize == self.site_events_.len() {
                self.process_circle_event(&mut output)?;
            } else if vp::event_comparison_predicate::event_comparison_bif::<I>(
                &self.site_events_[site_event_iterator_ as usize],
    ...
    pub(crate) fn init_sites_queue(&mut self) -> vse::SiteEventIndexType {
        // Sort site events with the libstdc++ introsort permutation so
        // geometrically-equal duplicates dedup to the same survivor as
        // the C++ boost build (stable sort keeps insertion order and
        // diverges the whole downstream enumeration).
        crate::libstdcxx_sort::sort(&mut self.site_events_, &|lhs, rhs| {
            vp::event_comparison_predicate::event_comparison_bii::<I>(lhs, rhs)
        });
    ...
    pub(crate) fn init_beach_line(
        ...
        if self.site_events_.len() == 1 {
            ...
            *site_event_iterator_ += 1;
            ...
            while (*site_event_iterator_ as usize) < self.site_events_.len()
                && vp::is_vertical::<I>(
                    self.site_events_[*site_event_iterator_ as usize].point0(),
                    self.site_events_[0].point0(),
                )
                && self.site_events_[*site_event_iterator_ as usize].is_vertical()
            {
                *site_event_iterator_ += 1;
                skip += 1;
            }
    ...
    fn init_beach_line_default(
        ...
        let first = self.site_events_[first as usize];
        let second = *site_event_iterator_;
        let second = self.site_events_[second as usize];
    ...
    fn init_beach_line_collinear_sites(
        ...
        while it_second != *site_event_iterator_ {
            let first = &self.site_events_[it_first as usize];
            let second = &self.site_events_[it_second as usize];

            // Create a new beach line node.
            let new_node_key = vb::BeachLineNodeKey::<I>::new_2(*first, *second);
    ...
    fn deactivate_circle_event(
        &mut self,
        beachline_ptr: &BeachlineCursor<I>,
    ) -> Result<(), BvError> {
    ...
    pub(crate) fn process_site_event(
        ...
        let (mut right_it, last_index) = {
            ...
            let site_event = self
                .site_events_
                .get(*site_event_iterator_ as usize)
                .ok_or_else(|| {
            ...
            if !site_event.is_segment() {
                    ...
                    && self.end_points_.peek().unwrap().site() == site_event.point0()
                {
                    ...
                    let mut b_it = BeachlineCursor::new_2(
                        Rc::clone(&self.beach_line_.beach_line_),
                        *b_it.beachline_index(),
                    )?;
                    #[cfg(feature = "console_debug")]
                    {
                        self.beach_line_.dbgpa_compat_(&self.circle_events_)?;
                ...
                let mut last = self.site_events_.get(last_index as usize);
                while (last_index as usize) < self.site_events_.len()
                    && last.is_some()
                ...
                {
                    last_index += 1;
                    last = self.site_events_.get(last_index as usize);
                }
            }
        ...
        while *site_event_iterator_ != last_index {
            // site_event is a copy of the event site_event_iterator_ is indexing
            let mut site_event = self.site_events_[*site_event_iterator_ as usize];
            ...
            if !right_it.is_ok()? {
                ...
                self.activate_circle_event(
                    ...
                    *(right_it.get_k()?.right_site()),
                    right_it.current_must_be_valid()?,
                )?;
                right_it = left_it;
            } else {
                let (site_arc2, site3) = {
    ...
    pub(crate) fn process_circle_event(&mut self, output: &mut vd::Diagram) -> Result<(), BvError> {
        #[cfg(feature = "console_debug")]
        {
        ...
        // Change the (A, B) bisector node to the (A, C) bisector node.
        {
            ...
            let _ = it_first.replace_key(it_first_key_after)?;
        ...
        #[cfg(feature = "console_debug")]
        assert_ne!(
            it_first.current_must_be_valid()?,
            it_last.current_must_be_valid()?
        );

        it_last.remove_current()?;
        ...
        if !self.beach_line_.is_empty() && !it_first.is_at_head()? {
            ...
            self.activate_circle_event(site_l1, site1, site3, it_last.current_must_be_valid()?)?;
        }
        ...
        if it_last.is_ok()? {
            ...
            self.activate_circle_event(site1, site3, site_r1, it_last.current_must_be_valid()?)?;
        }
        Ok(())
    }
    ...
    fn insert_new_arc(
        &mut self,
        site_arc1: vse::SiteEvent<I>,
        site_arc2: vse::SiteEvent<I>,
        site_event: vse::SiteEvent<I>,
        position: Option<vb::BeachLineIndex>,
        output: &mut vd::Diagram,
        ...
        let new_left_node = vb::BeachLineNodeKey::<I>::new_2(site_arc1, site_event);
        let new_right_node =
            // Set correct orientation for the first site of the second node.
            if site_event.is_segment() {
                vb::BeachLineNodeKey::<I>::new_2(*site_event.clone().inverse(), site_arc2)
            } else {
                vb::BeachLineNodeKey::<I>::new_2(site_event, site_arc2)
            };
        ...
        if site_event.is_segment() {
            ...
            let new_node =
                vb::BeachLineNodeKey::<I>::new_2(site_event, *site_event.clone().inverse());
    ...
    fn activate_circle_event(
        &mut self,
        site1: vse::SiteEvent<I>,
        site2: vse::SiteEvent<I>,
        site3: vse::SiteEvent<I>,
        bisector_node: vb::BeachLineIndex,
    ) -> Result<(), BvError> {
        // Check if the three input sites create a circle event.
        if let Some(c_event) = vp::circle_formation_predicate::circle_formation::<I>(
            &site1,
            &site2,
            &site3,
            bisector_node,
            ...
            if let Some(mut bd) = bn.1.get() {
                ...
                return Err(BvError::InternalError(format!(
                    "activate_circle_event could not find node by key {bisector_node}",
                )));
            }
        }
        Ok(())
